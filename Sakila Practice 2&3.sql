...
select actor_id, count(distinct film_id) as count_difilmID, film_id
from film_actor
group by actor_id
order by 2 desc;

select actor_id, film_id, count(distinct film_id) as count_difilmID
from film_actor
group by actor_id
order by 3 desc;
...
#is this movie available on 25th May?
select * from rental;
...
# no result means it is available for rent.
